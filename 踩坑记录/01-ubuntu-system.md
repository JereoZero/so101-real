...
- 移动固态硬盘 ≥ 256GB（本项目使用 USB 3.0 接口外置盒 + NVMe SSD）
- 一个 ≥ 8GB U 盘（做启动盘用）
- 在另一台能上网的电脑上：
  - 下载 Ubuntu 22.04.5 ISO：https://releases.ubuntu.com/22.04/
  - 用 balenaEtcher 或 Rufus 将 ISO 写入 U 盘

### 安装时关键注意事项

1. **磁盘选择不能选错！** 安装类型选 "Erase disk and install Ubuntu" 后，务必确认选中的是移动固态硬盘，不是主机内置盘。根据磁盘容量区分
2. **登录凭据**：本项目设置用户名为 `jer`，密码自行设定
3. **启动顺序**：安装完成后重启，在 BIOS/UEFI 引导菜单中（F12/F2/Del）选择从 USB 移动硬盘启动
4. **首次启动可能黑屏**：如果显卡不是 NVIDIA 或兼容性差，需要在 GRUB 启动时按 e 进入编辑模式，在 `linux` 行添加 `nomodeset` 参数。详细步骤见 [1.1 移动硬盘启动黑屏](#11-移动硬盘启动黑屏nomodeset)
